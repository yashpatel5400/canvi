...
def assess_coverage(task, posterior, fn, device = "cpu", coverage_trials = 1_000, num_coverage_pts = 20):
    ...
    variational_coverages_per_size = [np.zeros(num_coverage_pts) for _ in range(sample_sizes)]
    # times = [[] for _ in sample_sizes]

    for j in range(coverage_trials):
        test_theta = calibration_prior(num_samples=1).to(device)
        test_x = calibration_simulator(test_theta).to(device)

        predicted_lps = posterior.log_prob(test_theta[0].view(1,-1).to(device), test_x[0].view(1,-1).to(device)).detach()
        ...
        for i, sample_size in enumerate(sample_sizes):
            ...
            predicted_lps = posterior.log_prob(test_x, empirical_theta_dist).detach()
            ...
            # times[i].append(calibration_time)
...
if __name__ == "__main__":
    ...
    # plot_posterior(task, encoder, fn)
    ...
    log_prob = encoder.log_prob(calibration_theta.to(device), calibration_x.to(device))
    for calibration_theta_pt, calibration_x_pt in zip(calibration_theta, calibration_x):
        log_prob = encoder.log_prob(calibration_theta_pt.view(1,-1).to(device), calibration_x_pt.view(1,-1).to(device))
        prob = log_prob.detach().cpu().exp().numpy()
        cal_scores.append(1 / prob)
    cal_scores = np.array(cal_scores)

    assess_coverage(task, encoder, fn, coverage_trials=50)
